...
class GerenciadorMarcas {
  ...
  cadastrarMarca(nomeMarca, configuracaoMarca) {
    ...
    if (configuracaoMarca.tarefas) {
      Object.entries(configuracaoMarca.tarefas).forEach(
        ([nomeTarefa, configuracaoMarca]) => {
  ...
  async executarTarefa(nomeMarca, nomeTarefa, contexto = {}) {
    ...
    if (!tarefa) {
      throw new Error(
        `Tarefa ${nomeTarefa} não encontrada para a marca ${nomeMarca}`,
      );
    }
    ...
    try {
      ...
      console.error(
        `Erro ao executar a tarefa ${nomeTarefa} da marca ${nomeMarca}: ${erro}`,
      );
      return {
        sucesso: false,
        nomeMarca,
...
export const gerenciadorMarcas = new GerenciadorMarcas();

if (typeof window !== 'undefined') {
  window.gerenciadorMarcas = gerenciadorMarcas;
  console.log('✅ GerenciadorMarcas inicializado');
}
